...
public class TableImpl implements Table, MetadataInfo, Ownable,
    ...
    public static final String DOT_KEY_TAG = "._key";
    ...
    public static final String DOT_BRACKETS = ".[]";


    public static final String FN_KEYS = "keys(";
    public static final String FN_KEYOF = "keyof(";
    public static final String DOT_KEYS = ".keys()";
    public static final String DOT_VALUES = ".values()";
     ...
     * the corresponding fields of given a RowImpl or PrimaryKeyImpl. The binary
     * prim key is given as a BinaryKeyIterator. The given "row" is associated
     * with a given "targetTable".
     *
    ...
    private boolean initRowFromKeyBytes(TableImpl targetTable,
        ...
        while (currentPrimKeyColumn.hasNext()) {
            ...
            int pkFieldPos = (row instanceof PrimaryKeyImpl ?
                              pos :
                              targetTable.getVersionInfo()
                              .getPrimKeyPositions()[pos]);
        ...
        if (data.length >= (offset + 1)) {
            ...
            try {
                ...
                SimpleAvroReader reader =
                    new SimpleAvroReader(writerSchema, getSchema(), row);
                reader.read(decoder);
                return true;
            } catch (Exception e) {
                /*
        ...
        for (int pos = 0; pos < row.getNumFields(); ++pos) {
            ...
            if (!info.isPrimKeyAtPos(pos)) {
                if (fme.hasDefaultValue()) {
                    row.put(pos, fme.getDefaultValue());
    ...
    public boolean rowFromValueVersion(ValueVersion vv, RowImpl row) {
        ...
        return initRowFromByteValue(row, data, format, 0);
    }
    ...
    private void validateEvolution(FieldMap newFields) {
        ...
        for (Index index : indexes.values()) {
            ...
            for (String field : ((IndexImpl)index).getFieldsInternal()) {
        ...
        if (asJson) {
            ...
            for (List<String> fieldPath : fieldPaths) {
                ...
                if (tablePath.getLastStep() == TableImpl.BRACKETS) {
                    ...
                    if (def != null) {
                        fnode.put(NAME, tablePath.getPathName());
                        def.toJson(fnode);
                        continue;
                    }
                 ...
                 */
                FieldMapEntry entry = getFieldMap().getFieldMapEntry(tablePath);
                fnode.put(NAME, tablePath.getPathName());
                entry.toJson(fnode);
            }
    ...
    static void validateIdentifier(String name,
        ...
        if (isId && (name.length() > MAX_ID_LENGTH)) {
            throw new IllegalCommandException
                ("Illegal name: " + name +
                    ". Table names must be less than or equal to " +
                    MAX_ID_LENGTH + " characters.");
        }
        if (!isId && (name.length() > MAX_NAME_LENGTH)) {
            throw new IllegalCommandException
                ("Illegal name: " + name +
    ...
    static FieldDefImpl findTableField(TablePath tablePath) {
        ...
        ListIterator<String> fieldPath = tablePath.iterator();
        assert fieldPath.hasNext();
        FieldDefImpl def =
            tablePath.getFieldMap().getFieldDef(fieldPath.next());
    ...
    /**
     * Translate from an external representation of a stringified path to the
     * internal one. There are 2 external formats: the "old" one (in R4.2)
     * and the "new" one (in R4.3 and later). Both external formats use "[]"
     * for array steps, and convert it to ".[]" in the internal format.
     * The 2 external formats differ in how the represent map steps:
     *
     * 4.0 external                 Internal
     * keyof(map_path)          =>  map_path._key
     * elementof(map_path)      =>  map_path.[]
     * elementof(map_path).path =>  map_path.[].path
     *
     * 4.2 external                 Internal
     * keys(map_path)           =>  map_path._key
     * map_path[]               =>  map_path.[]
     * map_path[].path          =>  map_path.[].path
     *
     * 4.3 external                 Internal
     * map_path.keys()          =>  map_path._key
     * map_path.values()        =>  map_path.[]
     * map_path.values().path   =>  map_path.[].path
     *
     * @return the internal format corresponding to the given external format
     * of a stringified path. Returns null if the field is illegal. The
     * caller handles this.
     */
    public static String translateFromExternalField(String field) {

        if (field.contains(DOT_BRACKETS)) {
            // this was already translated it shouldn't be called again
            return field;
        }

        String lower = field.toLowerCase();

        /* convert .values() steps to .[] steps */
        int mapValuesIdx = lower.indexOf(DOT_VALUES);
        if (mapValuesIdx >= 0) {
            StringBuilder sb = new StringBuilder();
            sb.append(field.substring(0, mapValuesIdx));
            sb.append(DOT_BRACKETS);
            sb.append(field.substring(mapValuesIdx + DOT_VALUES.length(),
                                      field.length()));
            field = sb.toString();
            lower = field.toLowerCase();

        } else if (lower.contains(BRACKETS)) {
            /* convert [] steps to .[] steps */
            field = field.replace(BRACKETS, DOT_BRACKETS);
            lower = field.toLowerCase();

        } else if (lower.contains(ELEMENT_OF)) {
            /* convert elementof(map_path) steps to .[] steps */
            StringBuilder sb = new StringBuilder();
            int lp = field.indexOf('(');
            assert lp >= 0; /* ELEMENTOF guarantee this */
        ...
        /*
         * convert a .keys() step to a ._key step (there can be at most
         * one such step.
         */
        int mapKeysIdx = lower.indexOf(DOT_KEYS);
        if (mapKeysIdx >= 0) {
            StringBuilder sb = new StringBuilder();
            sb.append(field.substring(0, mapKeysIdx));
            sb.append(DOT_KEY_TAG);
            sb.append(field.substring(mapKeysIdx + DOT_KEYS.length(),
                                      field.length()));
            field = sb.toString();
            return field;
        }

        /* convert keys(map_path) to map_path._key */
        boolean isMapKeys = (lower.startsWith(FN_KEYS) ||
                             lower.startsWith(FN_KEYOF));

        if (isMapKeys) {
            StringBuilder sb = new StringBuilder();
            int lp = field.indexOf('(');
            assert lp >= 0; /* KEYS guarantees this */
            int rp = field.indexOf(')', lp);
            if (rp == -1) {
                return null;
            }
    ...
    private static class SimpleAvroReader {
        ...
        private FieldValue read(Schema schema,
            ...
            case ARRAY:   return readAvroArray(schema, def, in);
            case MAP:     return readAvroMap(schema, def, in);
            case UNION:
                Schema effectiveType = schema.getTypes().get(in.readIndex());
                return read(effectiveType, def, in);
        ...
        private MapValueImpl readAvroMap(Schema schema,
                                         FieldDefImpl def,
                                         Decoder in) throws IOException {
            ...
            for (long i = in.readMapStart(); i != 0; i = in.mapNext()) {
                for (long j = 0; j < i; j++) {
                    ...
                    map.put(key, read(valueType, mdef.getElement(), in));
                }
            }
            return map;
        }

        private ArrayValueImpl readAvroArray(Schema schema,
                                             FieldDefImpl def,
                                             Decoder in) throws IOException {
        ...
        private FieldValueImpl readTimestamp(FieldDefImpl def, Decoder in)
            throws IOException {

            return ((TimestampDefImpl)def)
                        .createTimestamp(in.readBytes(null).array());
        }

        private FieldValueImpl readNumber(Decoder in)
            throws IOException {

            return numberDef.createNumber(in.readBytes(null).array());
        }
    }
    ...
    private void writeAvroRecord(Encoder encoder,
        ...
        for (int pos = 0; pos < fieldMap.size(); ++pos) {
            ...
            if (!isRow || !info.isPrimKeyAtPos(pos)) {
                 ...
                 */
                writeAvro(encoder, fv, fme.getFieldDef());
            }
        }
    }
    ...
    private void writeAvroMap(Encoder encoder,
         ...
         * from the FieldValue.
         */
        FieldDef elementDef = mapValue.getElementDef().isJson() ?
            mapValue.getElementDef() : null;
        ...
        for (Map.Entry<String, FieldValue> entry :
            ...
            writeAvro(encoder, (FieldValueImpl) entry.getValue(), elementDef);
        }
        encoder.writeMapEnd();
    }
    ...
    private void writeAvroArray(Encoder encoder,
         ...
         * from the FieldValue.
         */
        FieldDef elementDef = arrayValue.getElementDef().isJson() ?
            arrayValue.getElementDef() : null;
        encoder.writeArrayStart();
        encoder.setItemCount(arrayValue.size());
        for (FieldValue fv : arrayValue.getArrayInternal()) {
            encoder.startItem();
            writeAvro(encoder, (FieldValueImpl) fv, elementDef);
        }
    ...
    private static void serializeJson(Encoder encoder,
        ...
        FieldValueSerialization.writeFieldValue(fieldValue, true,
                                                out, (short) 0);
     ...
     */
    private static FieldValueImpl deserializeJson(Decoder in)
            throws IOException {

        byte[] bytes = in.readBytes(null).array();
        final ByteArrayInputStream bais = new ByteArrayInputStream(bytes);
        final DataInput din = new DataInputStream(bais);
        FieldValueImpl newValue = (FieldValueImpl)
            FieldValueSerialization.readFieldValue(null, din, (short) 0);
        return newValue;
    }
     ...
     * table. This is the maximum of the serialVersions the columns
