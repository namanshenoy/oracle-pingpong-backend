...
public class TableBuilderBase {
    ...
    public TableBuilderBase addBinary(String name,
        ...
        return addField(name, def, nullable, null);
    }
    ...
    public TableBuilderBase addField(
        ...
        assert tablePath.concatPathSteps() != null;

        List<String> stepsList = tablePath.getSteps();
        assert stepsList != null && !stepsList.isEmpty();

        String pathName = tablePath.concatPathSteps();
        String newFieldName = stepsList.remove(stepsList.size() - 1);
        ...
        /*
         * A new TablePath is needed because the stepsList has been modified
         * by removing the last step, above.
         */
        tablePath = new TablePath(fields, stepsList);
        ...
        if (def == null) {
            throw new IllegalArgumentException(
                ...
                tablePath.concatPathSteps() + " because that path does not exist");
        }

        if (!def.isRecord()) {
            throw new IllegalArgumentException(
                "Can not add field " + newFieldName + " to path " +
                tablePath.concatPathSteps() +
    ...
    void validateFieldAddition(final String fieldName,
        ...
        if (fieldName != null) {
            TableImpl.validateIdentifier(fieldName, false);
        }
    }
    ...
    public void removeField(TablePath tablePath) {
        ...
        if (toBeRemoved == null) {
            throw new IllegalArgumentException
                ("Field does not exist: " + tablePath.concatPathSteps());
        }
