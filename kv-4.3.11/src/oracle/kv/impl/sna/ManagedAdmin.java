...
public class ManagedAdmin extends ManagedService {
    ...
    public String getDefaultJavaArgs(String overrideJvmArgs) {
        ...
        if ((ap == null) || (ap.getMap() == null)) {
            if (kvSNDir != null) {
                sb.append(" ").append(AdminService.HEAP_DUMP_PATH_ARG)
                    .append(kvSNDir.getAbsolutePath());
            }
            return sb.toString();
        }
        /* Heap dump to the log directory */
        if (kvRootDir != null && kvName != null) {
            final File loggingDir = FileNames.getLoggingDir(kvRootDir, kvName);
            sb.append(" ").append(AdminService.HEAP_DUMP_PATH_ARG)
                .append(loggingDir.getAbsolutePath());
        }
