        ...
        public void handleNotification(Notification notif,
            ...
            try {
                ...
                if (metrics != null && !metrics.isEmpty()) {
                    if (RepNode.NOTIFY_RN_OP_METRIC.equals(notif.getType())) {
                        recorder.record(CollectorRecorder.MetricType.RNOP, 
                                        metrics);
                    }
                    if (RepNode.NOTIFY_RN_EXCEPTION_METRIC.equals(
                            notif.getType())) {
                        recorder.record(
                            CollectorRecorder.MetricType.RNEXCEPTION, metrics);
                    }
                    if (RepNode.NOTIFY_RN_ENV_METRIC.equals(notif.getType())) {
                        recorder.record(CollectorRecorder.MetricType.RNENV,
                                        metrics);
                    }
                    if (RepNode.NOTIFY_RN_STATUS_CHANGE.equals(
                            notif.getType()) ||
                        RepNode.NOTIFY_RN_REPLICATION_STATE.equals(
                            notif.getType())) {
                        recorder.record(CollectorRecorder.MetricType.RNEVENT,
                                        metrics);
                    }
                    if (Admin.NOTIFY_PLAN_STATUS_CHANGE.equals(
                            notif.getType())) {
                        recorder.record(CollectorRecorder.MetricType.PLAN,
                                        metrics);
                    }
                }
            } catch (Exception e) {
