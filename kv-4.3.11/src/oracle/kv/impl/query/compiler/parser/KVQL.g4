...
from_clause : FROM name_path (AS? tab_alias)? (COMMA expr (AS? var_name))*;
...
select_clause :
    SELECT hints? ( STAR | (expr col_alias (COMMA expr col_alias)*) ) ;
...
/*
 * index_path handles a basic name_path but adds KEYS() and []
 * expressions to handle addressing in maps and arrays.
 * NOTE: if the syntax of KEYS or [] changes the source should be checked
...
id :
    (ACCOUNT | ADD | ADMIN | ALL | ALTER | AND | ANY_T| ANYATOMIC_T |
     ANYJSONATOMIC_T | ANYRECORD_T | AS | ASC | BY |
     CASE | CAST | COMMENT | CREATE | DECLARE | DEFAULT | DESC | DESCRIBE |DROP|
     ELEMENTOF | ELSE | END | ES_SHARDS | ES_REPLICAS | EXISTS |
     FIRST | FROM | FULLTEXT |
     GRANT | IDENTIFIED | IF | INDEX | INDEXES | IS | JSON | KEY | KEYOF | KEYS|
     LIFETIME | LAST | LIMIT | LOCK | MODIFY | NOT | NULLS |
     OF | OFFSET | ON | OR | ORDER | OVERRIDE |
     PASSWORD | PRIMARY | ROLE | ROLES | REVOKE |
     SELECT | SHARD | SHOW | TABLE | TABLES | THEN | TIME_UNIT | TO | TTL |
     TYPE | UNLOCK | USER | USERS | USING | VALUES | WHEN | WHERE |
