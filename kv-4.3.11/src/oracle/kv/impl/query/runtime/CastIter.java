 ...
 *
 * c. Casting table:
 *         to BOOLEAN DOUBLE FLOAT INTEGER LONG STRING ENUM BINARY FIXED_BINARY
 * from
 * BOOLEAN       x                                x
 * DOUBLE               x      x      x     x     x
 * FLOAT                x      x      x     x     x
...
public class CastIter extends PlanIter {
    ...
    static private class CastIterState extends PlanIterState {
        ...
        void checkQuantifier(ExprType.Quantifier quantifier) {
            switch (state) {
            case -1:
            ...
            case 0:
                if (quantifier == ExprType.Quantifier.PLUS ||
                    ...
                    throw new QueryException("Input not valid with type " +
    ...
    public boolean next(RuntimeControlBlock rcb) {
        ...
        state.checkQuantifier(theTargetQuantifier);
            ...
            case MAP: {
                ...
                for (Map.Entry<String, FieldValue> entry :
                    ...
                    FieldValueImpl elem = castValue(
                        (FieldValueImpl) entry.getValue(), elemDef, loc);
